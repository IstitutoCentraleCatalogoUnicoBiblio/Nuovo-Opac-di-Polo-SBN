...
if (typeof window.AudioContext !== "undefined") {
    console.log('Detected Chrome AudioContext');
    return new window.AudioContext();
} else if (typeof webkitAudioContext !== "undefined") {
    console.log('Detected Safari webkitAudioContext');
    return new window.webkitAudioContext();
} else if (typeof window.mozAudioContext !== "undefined") {
    console.log('Detected Mozilla mozAudioContext');
    return new window.mozAudioContext();
} else {
   console.error('Nessun AudioContext identificato !');
}
}
function initAudio() {
   audioCtx = audioContextCheck() ;
   if (audioCtx == undefined) {
	   console.error("Audio non supportato dal tuo browser");
	   return;
   }
...
(function ($) {
    ...
    $.fn.midiPlayer = function (options) {
        ...
        $.fn.midiPlayer.pause = function () {
            pause(); //midiPlayer_pause.style.display
        };
        $.fn.midiPlayer.isPlaying = function () {
            return (_EM_signalStop == 2) ;
        };
        $.fn.midiPlayer.playFromPause = function () {
            play();
        };
        $.fn.midiPlayer.getPlayingPercent = function () {
          //  stop();
          return midiPlayer_progress.style.width;
        };
        // Create the player
